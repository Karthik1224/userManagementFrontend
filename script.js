...
document.getElementById('loginForm').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch('https://usermanagement-production-d69d.up.railway.app/api/v1/auth/signin', {
            method: 'POST',
            headers: {
...
async function fetchAndDisplayCustomers(pageNo) {
    ...
    try {
        const response = await fetch('https://usermanagement-production-d69d.up.railway.app/api/v1/customer/fetchAll', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', async function() {
            ...
            if (confirm('Are you sure you want to delete this customer?')) {
                try {
                    const deleteResponse = await fetch(`https://usermanagement-production-d69d.up.railway.app/api/v1/customer/delete?phone=${encodeURIComponent(phone)}`, {
                        method: 'DELETE',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': 'Bearer ' + localStorage.getItem('jwtToken')
...
document.getElementById('customerForm').addEventListener('submit', async function(event) {
    ...
    try {
        const url = document.getElementById('formTitle').textContent === 'Add Customer'
            ? 'https://usermanagement-production-d69d.up.railway.app/api/v1/customer/add'
            : 'https://usermanagement-production-d69d.up.railway.app/api/v1/customer/update';
...
document.getElementById('syncButton').addEventListener('click', async function() {
    try {
        const response = await fetch('https://usermanagement-production-d69d.up.railway.app/api/v1/customer/sync', {
            method: 'POST',
            headers: {
                'Authorization': 'Bearer ' + localStorage.getItem('jwtToken')
            }
